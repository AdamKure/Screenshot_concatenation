...
def check_if_reversed_order(datetime1: str, datetime2: str) -> bool:
    """Returns True if datetime2 precedes datetime1"""
    assert (
        datetime1 is not None and datetime2 is not None
    ), "One of the datetimes is missing"
    assert datetime1 != datetime2, "Bozh datetimes are the same"

    return datetime1 > datetime2


def create_image_pairs(
    image_path_list: List[str], spam: bool = False
...
def create_new_image(
    ...
    """
    try:
        if is_sorted_by_time(image1, image2):
            image1, image2 = image2, image1
    except AssertionError:
        print("Sorting failed for images:", image1, image2, sep="\n")
        return
...
def get_readable_exif_data(
    image: Image,
) -> Union[Dict[str, Union[str, int, bytes]], None]:
    """
...
def process_image_pair(arg_tupple: Tuple[Tuple[str], str]):
    ...
    with Image.open(image_pair[0]) as image1, Image.open(image_pair[1]) as image2:
        create_new_image(
            image1, image2, output_folder_path=output_folder_path, print_enabled=True
        )


def is_sorted_by_time(image1: Image, image2: Image) -> bool:
    ...
    return check_if_reversed_order(time1, time2)
...
def stich_screenshots(image1: Image, image2: Image) -> Image:
    ...
    new_image.paste(
        image1.crop((0, 0, image1.width, image1.height - overlap[0])), (0, 0)
    )
    new_image.paste(
        image2.crop((0, overlap[1], image2.width, image2.height)),
        (0, image1.height - overlap[0]),
    )
    return new_image
...
if __name__ == "__main__":
    ...
    # added late for multiprocessing compatibility
    arg_list = [(path, output_folder_path) for path in image_pair_list]
    with Pool() as pool:
